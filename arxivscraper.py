...
class Scraper(object):
    ...
    def __init__(self, category, date_from=None, date_until=None, t=15, filters={}):
    ...
    def store(self,output,file_name):
        cols = ('id', 'title', 'categories', 'abstract', 'doi', 'created', 'updated', 'authors')
        df = pd.DataFrame(output,columns=cols)
        df.to_pickle(file_name)
        print ('Stored as ' + file_name)
    ...
    def scrape(self):
        ...
        df = self.store(ds,self.cat + '_' + self.f + '_' + self.u)
